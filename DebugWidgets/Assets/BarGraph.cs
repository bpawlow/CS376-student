...
public class BarGraph : MonoBehaviour
{
    ...
    public void SetReading(float value)
    {
        ...
        SetWidthPercent(value / Math.Max(Math.Abs(Max), Math.Abs(Min)), color);


        // TODO: Update the text to read: {name} : {value}

        Text.text = name + " : " + actualValue;
    ...
    public void SetWidthPercent(float value, Color c)
    {
        ...
        if (value == 0)
        {
            BarTransform.localScale = new Vector3(0, BarTransform.localScale.y, BarTransform.localScale.z);
        }
        else if (value == 1)
        {
            if (signedDisplay)
            {
                BarTransform.localScale = new Vector3(BarTransform.localScale.y / 2, BarTransform.localScale.y, BarTransform.localScale.z);
            }
            else
            {
                BarTransform.localScale = new Vector3(BarTransform.localScale.y, BarTransform.localScale.y, BarTransform.localScale.z);
            }
        }
        else if (value < 0) { 
            ...
            {
                BarTransform.localScale = new Vector3(-(BarTransform.localScale.y) / 2, BarTransform.localScale.y, BarTransform.localScale.z);
                //BarTransform.localPosition = new Vector3(BarTransform.localPosition.x * 2, BarTransform.localPosition.y, BarTransform.localPosition.z);
            }
            else
            {
                BarTransform.localScale = new Vector3(value / 2, BarTransform.localScale.y, BarTransform.localScale.z); //not sure
            }

        }
        else
        {
            BarTransform.localScale = new Vector3(value / 2, BarTransform.localScale.y, BarTransform.localScale.z);
        }
    }
    ...
    public static BarGraph Find(string name, Vector2 position, float min, float max)
    {
        ...
        else
        {
            ...
            var bgComponent = go.GetComponent<BarGraph>();  // Change null here
